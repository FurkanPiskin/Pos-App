...
const onFinish = (values) => {
  try {
    fetch(`https://localhost:7211/api/Product/${editingItem.productId}`, {
      method: "PUT",
      body: JSON.stringify({ ...values, productId: editingItem.productId }),
...
const deleteCategory = (id) => {
  if (window.confirm("Emin misiniz?")) {
    try {
      fetch(`https://localhost:7211/api/Product/${id}`, {
        method: "DELETE",
        headers: { "Content-type": "application/json; charset=UTF-8" },
      });
        ...
        return (
          <div>
            ...
            <Button
              type="link"
              danger
              onClick={() => deleteCategory(record.productId)}
            >
              Sil
            </Button>
          </div>
